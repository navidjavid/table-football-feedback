...
Use the following task description when implementing the server:

```text
Implement a Flask + SQLite Raspberry Pi server for this table football project.

Current Pico firmware sends HTTP POST requests to SERVER_IP:SERVER_PORT at endpoint /update.
The Pico keeps the embedded game logic. The Raspberry Pi server must handle dashboard, database, registration, guest players, and player match history.

Create a server folder with app.py, football.db generated automatically, templates/index.html, static/style.css, static/script.js, and README.md.

Use Flask and SQLite only.

Dashboard must be one page only and must include:
1. Live Match section
2. All Players list
3. Player Profile dialog with match history
4. Register Guest dialog

Do not include leaderboard, recent matches section, total shots, last goal, possession, or ball position.

Implement database tables: players, matches, live_state.
...
POST /update should receive Pico JSON, find or create players by RFID UID, update live_state, and save a finished match once when state is GAME_OVER.

Unknown RFID UIDs should automatically create Guest1, Guest2, Guest3, etc.

Registration should update the existing guest row, not create a new player, so all guest match history remains connected.

The frontend should poll /api/live every second and /api/players every three seconds.

Clicking a player in All Players should open the profile dialog and call /api/player/<player_id>.

If a player is a guest, the profile dialog should show Register This Player. That opens the registration dialog.

After registration, refresh live match, players list, and profile dialog.

Keep everything simple, readable, and suitable for a university prototype.
```
